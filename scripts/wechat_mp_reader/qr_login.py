...
def _start_login(session: requests.Session) -> str:
    session.cookies.set("uuid", _generate_uuid())
    ...
    return resp.cookies.get("uuid") or resp.headers.get("X-UUID") or ""
...
def _check_login_status(session: requests.Session, state: dict[str, Any]) -> dict[str, Any]:
    fingerprint = session.cookies.get("fingerprint") or _generate_uuid()
    params = {
        "action": "ask",
...
def _extract_login_info(session: requests.Session) -> dict[str, str]:
    ...
    token = token_match.group(1) if token_match else session.cookies.get("token", "")
    cookies = requests.utils.dict_from_cookiejar(session.cookies)
